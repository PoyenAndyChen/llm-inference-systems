...
| `PIECEWISE` | Capture decode steps only; was the prior default before V1 |
...
vLLM is among the most accessible references for studying these design choices: the codebase is open, the abstractions are well-named, and the research-to-production pipeline moves quickly. The design decisions described in this chapter are directly traceable in the source. For practitioners building or evaluating inference infrastructure, the vLLM V1 source remains a primary reference for the current state of production LLM serving. The chapters that follow examine alternative engines (TRT-LLM, SGLang) and specialized hardware runtimes — but the architectural vocabulary introduced here (token budget, KVConnector, paged blocks, prefix hash chains, BGMV kernels) is the common language those comparisons will use.
