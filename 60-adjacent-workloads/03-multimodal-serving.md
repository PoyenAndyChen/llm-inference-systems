...
The companion chapter on RAG serving ([§60/04](04-rag-infrastructure.md)) treats retrieval as a pre-prefill stage that injects extra text tokens. Multimodal serving generalizes the move: image, audio, and video inputs each pass through a *non-text encoder* before they enter the standard prefill–decode pipeline. That third stage is large enough — both in compute and in tokens emitted — to merit its own design vocabulary, its own caching layer, and in production its own GPUs. This chapter develops the EPD model, walks the encoder caching and disaggregation lineage, treats audio and video as variants of the same model, and ends with the real-time streaming voice pipeline that has crystallized as the latency frontier of multimodal serving in 2026.
...
Caveat: the FastVLM speedups are reported on a specific LLM backbone size and a specific benchmark suite. The general claim — that token count drives TTFT linearly through prefill — is robust; the specific multipliers do not transfer unchanged to other backbones. Practitioners should re-measure on their own model.
...
Caveat: TTFA numbers from each vendor are vendor-supplied and measured in ideal network conditions. End-to-end production TTFA over open internet routinely sits 50–150 ms above any single vendor's claim. Sub-300 ms TTFA is a feasible architectural target on managed network paths in 2026, achieved by integrated stacks (GPT-4o realtime) and by carefully composed disaggregated stacks (LiveKit + best-of-breed providers); it is not a guarantee for any specific deployment.
