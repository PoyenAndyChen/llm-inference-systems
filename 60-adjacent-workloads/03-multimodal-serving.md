...
The previous chapter on RAG serving ([see §60/04-rag-infrastructure](04-rag-infrastructure.md)) treated retrieval as a pre-prefill stage that injects extra text tokens. Multimodal serving generalizes the move: image, audio, and video inputs each pass through a *non-text encoder* before they enter the standard prefill–decode pipeline. That third stage is large enough — both in compute and in tokens emitted — to merit its own design vocabulary, its own caching layer, and in production its own GPUs. This chapter develops the EPD model, walks the encoder caching and disaggregation lineage, treats audio and video as variants of the same model, and ends with the real-time streaming voice pipeline that has crystallized as the latency frontier of multimodal serving in 2026.
