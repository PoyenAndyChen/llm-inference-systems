...
The previous chapter named the metrics. This one derives the arithmetic that connects those metrics to hardware. The body of techniques cataloged later in the book is, in a strong sense, *the same equation rearranged* — different ways of moving the operating point of a request along a roofline whose axes are FLOPs and HBM bytes. Doing this arithmetic on the back of an envelope is a prerequisite for sizing capacity, debugging a regression, or evaluating a new technique's claimed speedup.
