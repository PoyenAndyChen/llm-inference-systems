...
The full hardware spec table — Hopper, Blackwell, Rubin, Rubin CPX, NVIDIA Groq 3 LPX, MI300X/MI355X, TPU v7 Ironwood, Trainium2/3 — is in [§70/01-nvidia-roadmap](../70-hardware/01-nvidia-roadmap.md). The numbers above use H100 SXM5 and B200 SXM as reference points; the same arithmetic applied to other accelerators gives an immediate first-order capacity estimate.
